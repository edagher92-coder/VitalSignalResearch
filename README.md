...
For continued development, start with [the Cursor/cross-PC handoff](docs/CURSOR_HANDOFF.md), then read [the session handoff](docs/SESSION_HANDOFF.md), [threat model](docs/THREAT_MODEL.md), [contribution gates](CONTRIBUTING.md) and [security policy](SECURITY.md). Cursor applies the repository's fail-closed rules from `.cursor/rules/evidessa-research.mdc` automatically when the repository root is opened.
