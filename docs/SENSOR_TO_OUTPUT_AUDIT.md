...
- **Measured:** a qualified observation from a validated physical adapter. Version 0.5 has no such personal observations yet.
...
- Lock every forecast before the outcome is entered and prohibit future leakage. Version 0.5 implements only the frozen +72h-to-+73h point-assessment contract; a next-day/24-hour endpoint would require its own frozen schema, outcome instrument and validation.
...
| Fatigue/function trajectory | Frozen prospective outcome scale, prior fatigue/function, sleep, activity, workload-response and qualified physiology | Version 0.5 research endpoint: point assessment at +72h to +73h; any next-day endpoint is separate and not implemented | After prospective validation, a calibrated estimate/interval that beats preregistered persistence and simple-feature baselines | Missing outcome history, model drift, uncalibrated subgroup, concern hold, weak coverage or failure to outperform baselines |
